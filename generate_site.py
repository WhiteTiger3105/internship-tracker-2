#!/usr/bin/env python3
"""Genereaza docs/index.html din data/results.json + data/profile.json."""
import json
...
QUALIFICATION_NOTE = {
    "engleza": "Ai engleza B2 Cambridge - cerinta indeplinita.",
    "english": "Ai engleza B2 Cambridge - cerinta indeplinita.",
    "germana": "Ai germana functionala - diferentiator, mai ales pt. Wolf Theiss/Schoenherr.",
    "german": "Ai germana functionala - diferentiator, mai ales pt. Wolf Theiss/Schoenherr.",
    "an iii": "Atentie: cerinta tipica An III/IV - tu esti An II, verifica daca accepta exceptii.",
    "an iv": "Atentie: cerinta tipica An III/IV - tu esti An II, verifica daca accepta exceptii.",
    "anul iii": "Atentie: cerinta tipica An III/IV - tu esti An II.",
    "anul iv": "Atentie: cerinta tipica An III/IV - tu esti An II.",
}


def build_card(item):
    region = item["regiune"]
    priority_class = "priority" if item.get("priority") else ""
    tag_priority = '<span class="tag priority">M&A / Corporate / Civil / Imobiliare</span>' if item.get("priority") else ""

    quals_html = ""
    quals = item.get("qualifications_detected") or []
    if quals:
        notes = [QUALIFICATION_NOTE.get(q, f"Cerinta detectata: {q}") for q in quals]
        quals_html = '<div class="quals"><b>Match cu profilul tau:</b><br>' + "<br>".join(notes) + "</div>"

    return f"""
    <div class="card {priority_class}" data-region="{region}">
      <span class="region">{region}</span>
      <h3>{item['firma']}</h3>
      {tag_priority}
      <div class="snippet">{item['text']}</div>
      {quals_html}
      <div><a class="src" href="{item['link']}" target="_blank" rel="noopener">Vezi sursa &rarr;</a></div>
    </div>
    """
...
def main():
    ...
    if not results_path.exists():
        data = {"generated_at": datetime.now().isoformat(), "total_found": 0, "results": []}
    else:
        data = json.loads(results_path.read_text(encoding="utf-8"))

    cards_html = "".join(build_card(r) for r in data["results"]) or '<div class="empty">Niciun anunt gasit in ultima rulare. Verifica din nou maine.</div>'
...
<title>Internship/Stagiu Avocatura - Bucuresti si Iasi</title>
<style>{CSS}</style>
</head>
<body>
<header>
  <h1>Internship / Stagiu de practica - Avocatura</h1>
  <p>Bucuresti si Iasi - Prioritizat: Corporate/M&A, Civil, Imobiliare</p>
  <div class="meta">Ultima actualizare: {data['generated_at']} - {data['total_found']} rezultate gasite</div>
</header>
<div class="controls">
...
"""
    ...
    print(f"Pagina generata: {DOCS_DIR / 'index.html'}")
